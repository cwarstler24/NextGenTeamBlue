...
def test_index_page_ok(client, monkeypatch, loguru_capture):
    ...
    monkeypatch.setattr(builtins, "open", _fake_open_with("<h1>Hello</h1>"), raising=True)
...
def test_index_test_page_ok(client, monkeypatch, loguru_capture):
    ...
    monkeypatch.setattr(builtins, "open", _fake_open_with("<p>ok test</p>"), raising=True)
...
def test_redirect_index_test_html(client):
    r = client.get("/index_test.html/", follow_redirects=False)
