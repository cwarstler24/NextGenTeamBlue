...
def test_add_resource_type_allows_manager_and_returns_200(monkeypatch):
    ...
    def fake_exec(q, p=None):
        calls["query"] = q; calls["params"] = p; return {"status":"success", "rows_affected":1}
    monkeypatch.setattr(dc.database_connector, "execute_query", fake_exec)
    status = dc.add_resource_type("Manager", {"asset_type_name": "Laptop"})
    assert status == 200
...
def test_add_resource_asset_creates_and_updates(monkeypatch):
    ...
    monkeypatch.setattr(dc.database_connector, "execute_query", fake_exec)
    status = dc.add_resource_asset("Manager", {"type_id": 1, "location_id": 2, "employee_id": None, "notes": "", "is_decommissioned": 0})
    assert status == 200
    # sanity: we called INSERT, SELECT (id), SELECT (type name), UPDATE in some order
...
def test_delete_resource_updates_flag(monkeypatch):
    called = {}
    monkeypatch.setattr(dc.database_connector, "execute_query", lambda q, p=None: {"status":"success"})
    status = dc.delete_resource("Manager", 10)
    assert status == 200

def test_update_resource_updates_fields(monkeypatch):
    monkeypatch.setattr(dc.database_connector, "execute_query", lambda q, p=None: {"status":"success"})
    status = dc.update_resource("Manager", {"asset_id":1,"type_id":2,"location_id":3,"employee_id":4,"notes":"","is_decommissioned":0})
    assert status == 200
...
def test_get_resources_returns_tuple(monkeypatch):
    ...
    code, items = dc.get_resources()
    assert code == 200 and items == [{"id":1}]

def test_get_resource_types_returns_tuple(monkeypatch):
    monkeypatch.setattr(dc.database_connector, "execute_query", lambda q, p=None: [{"id":1,"asset_type_name":"Laptop"}])
    code, items = dc.get_resource_types()
