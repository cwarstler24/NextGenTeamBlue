...
describe('useAssetLocations', () => {
  ...
  it('initial state is correct', () => {
    ...
    expect(assetLocations.value).toEqual([])
    expect(assetLocationMap.value).toEqual({})
   ...
   * @description Verifies that fetchAssetLocations returns an error when no bearer token is present in localStorage.
   */
  it('returns error when no bearer token set and does not call API', async () => {
    const { fetchAssetLocations, error, isLoading } = useAssetLocations()
  ...
  it('adds "Bearer " prefix when token does not start with bearer', async () => {
    ...
    mockedAxiosGet.mockResolvedValueOnce({
      data: [
        { id: 1, asset_location_name: 'Building A' },
        { id: 2, asset_location_name: 'Building B' },
      ],
    })

    const { fetchAssetLocations } = useAssetLocations()
    await fetchAssetLocations()
  ...
  /**
   * @function successful fetch populates assetLocations and assetLocationMap
   * @description Verifies that a successful API call correctly populates the reactive state and lookup map.
   */
  it('successful fetch populates assetLocations and assetLocationMap', async () => {
    localStorage.setItem('bearerToken', 'abc123')

    const payload = [
      { id: 10, asset_location_name: 'Warehouse 1' },
      { id: 20, asset_location_name: 'Office Floor 3' },
    ]
    ...
    expect(error.value).toBeNull()
    expect(isLoading.value).toBe(false)
    expect(assetLocations.value).toEqual(payload)
    expect(assetLocationMap.value).toEqual({
      10: 'Warehouse 1',
      20: 'Office Floor 3',
    })
    expect(getAssetLocationName(10)).toBe('Warehouse 1')
    expect(getAssetLocationName(999)).toBe('Location 999')
  })
  ...
  it('sets generic error message on failed fetch (no detail)', async () => {
    ...
    expect(error.value).toBe('Failed to fetch asset locations')
    expect(isLoading.value).toBe(false)
  })
  ...
  it('isLoading toggles correctly during request', async () => {
    ...
    const { fetchAssetLocations, isLoading } = useAssetLocations()

    const p = fetchAssetLocations()
